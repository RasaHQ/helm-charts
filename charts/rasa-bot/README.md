...
The default configuration of the Rasa Bot chart deploys a Rasa Open Source Server, downloads a model, and serves the downloaded model.
...
The first time you install Rasa Bot, you may not have a model server available yet, or you may want an lightweight model for testing the deployment. For this purpose, you can choose between training or downloading an initial model. By default, the Rasa Bot chart downloads an example model from GitHub. To use this option, you don't have to change anything.
...
Any Rasa Open Source server can stream events to Rasa X/Enterprise using an [event broker](). Both Rasa Bot and Rasa X/Enterprise will need to refer to the  same event broker.
...
For example, to configure Rasa Bot to refer to the event broker started by Rasa X/Enterprise:
...
In the [`examples/rasa-bot`](../../examples) directory you can find more detailed examples of configuration:
